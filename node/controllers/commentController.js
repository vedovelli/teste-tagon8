...
exports.list = function(postid, callback) {
	...
	db.comment.find({'postid': postid}, function(error, comments) {
		...
		if (error) {
			callback({'comment': {errorMsg: 'Não foi possível listar os comentários'}});
		} else {
...
exports.get = function(id, callback) {
	...
	db.comment.findById(id, function(error, comment) {
		...
		if (error) {
			callback({'comment': {errorMsg: 'Não foi possível retornar o comentário'}});
		} else {
			callback(comment);
		}
	});
...
exports.save = function(postid, fullname, email, comment, callback) {
	...
	new db.comment({
		...
		if(err) {
			callback({'comment':{errorMsg: 'Não foi possível salvar o comentário'}});
		} else {
